class Timer {
    ...
    constructor(span, order = null) {
        if(order == null) {
            this.startTimer(span);
        } else {
            this.startOffsetTimer(span, order);
        }
    }
    //Timer für Span starten
    startTimer = (timeSpan, offsetMinutes = 0, offsetSeconds = 0) => {
        ...
        setInterval(() => {
            ...
            secondsLabel.innerHTML = this.pad(totalSeconds % 60); //Minuten aktualisieren
            minutesLabel.innerHTML = this.pad(parseInt(totalSeconds / 60)); //Sekunden aktualisieren
    ...
    startOffsetTimer = (span, order) => {
        ...
        this.startTimer(span, minutesSinceStart, secondsSinceStart);
    }
}