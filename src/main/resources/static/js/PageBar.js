class PageBar {
    ...
    async show() {
        let html = await fetch("pages/bar.html");
        let htmlContent = "";
        if (html.ok) {
            htmlContent = await html.text();
        }
        this._app.setPageContent(htmlContent);
        this.generateDeliveries(); //Ausstehende Lieferungen generieren
        for (let index = 0; index < 4; index++) {
            this.generateNextOrder(); //Alle offenen Bestellungen generieren
        }
        ...
        this.updateRfidConnection(false); //RFID Verbindungsanzeige initial Rot
    ...
    getFirstEmptyColumn = (count = 1) => { //count default beginnt bei 1
        ...
        } else return this.getFirstEmptyColumn(++count); //Funktion erneut durchführen
    };

    //Getränk fertig vorbereitet
    drinkDone = (drinkId) => {
        let drink = document.getElementById(drinkId);
        let toDoArea = drink.parentElement;
        let doneArea = this.getDoneArea(toDoArea); //Done Area ermitteln
        ...
        (async () => {
            ...
            this.updateRfidConnection(false);
            ...
            if(toDoArea.childElementCount == 1) {
                this.generateDeliveries(); //Neue Lieferung anlegen
                this.moveOrders(toDoArea); //Von Rechts nach Links aufrücken
            }
        })();
    };

    generateDeliveries = () => {
        //fetch("./resources/lieferung.json").then(
            ...
            data => {
                ...
                data.some(order => { //durch alle Bestellungen iterieren
                    if(tische.includes(order.tisch_id)) {
                        ...
                        this.newDelivery("Tisch " + order.tisch_id, order);
    ...
    newDelivery = (tablenumber, order=null) => {
        ...
        if(order != null) new Timer(spans[1], order); //Timer in zweitem Span starten
        if(order == null) new Timer(spans[1]);
    };

    //Alle Bestellungen rechts des fertiggestellten Bereichs nach links verschieben
    moveOrders = (finishedArea) => {
        if(finishedArea.id == "toDo4") { //Wenn der fertiggestellte Bereich ganz rechts ist
            ...
            this.generateNextOrder(); //Neue Bestellung aus Backlog generieren
        ...
        let doneArea = this.getDoneArea(finishedArea); //DoneArea zu finishedArea ermitteln
        ...
        this.moveOrders(nextToDoArea); //Rekursion --> Methode mit Rechter Spalte (nextToDoArea) wiederholen
    };

    //Nächste Spalten mit Bestellungen generieren
    generateNextOrder = () => {
        //fetch("./resources/orders.json").then(
        fetch("./api/getOffeneBestellungen").then( //fetch Spring Endpoint
            ...
            data => {
                let toDoArea = this.getFirstEmptyColumn();
                if(toDoArea == null) return false;
                data.some(order => { //durch alle Bestellungen iterieren
                    if(document.getElementById("drink_" + order.id) == null && order.status == 1){ //wenn die Bestellung noch nicht auf dem Frontend vorhanden ist
                        ...
                        this.generateOrdersForTable(data, order.tisch_id, toDoArea); //Alle offenen Bestellungen für den gewählten Tisch erzeugen
    ...
    generateOrdersForTable = (data, tisch_id, toDoArea) => {
        data.forEach(order => { //durch alle Bestellungen iterieren
            if(order.tisch_id == tisch_id) { //jede Bestellung mit der zuvor ermittelten TischId
                let newOrder = this.createNewOrder(order); //Neues Bestellungselement erzeugen
                if(order.status == 1) {
                    toDoArea.appendChild(newOrder); //Bestellung der Spalte hinzufügen
                }
                if(order.status == 2) {
                    this.getDoneArea(toDoArea).appendChild(newOrder);
                }
            }
        })
    }
    ...
    updateRfidConnection = (connected) => {
        if(connected) {
